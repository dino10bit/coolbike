...
export class OrderResolver {
  ...
  @Query(() => Order)
  async order(
    @Arg('invoiceNumber', () => ID) invoiceNumber: number,
    @Arg('state', () => OrderState, { nullable: true }) state: OrderState,
    @Ctx() ctx: Context
  ): Promise<Order> {
    const order = await OrderModel.findOne(state ? { state } : {})
      .where('invoiceNumber', invoiceNumber)
      .exec();

    if (!order) throw new Error(`order with invoiceNumber=${invoiceNumber} not found`);
  ...
  async createOrder(
    ...
    order.invoiceNumber = parseInt(String(Math.random()).substring(2,8));
    await order.save();
